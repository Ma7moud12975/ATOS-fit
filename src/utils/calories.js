...
const PER_MIN_BASE = {
  ...
  straightarmplank: 4.0, // kcal per minute (same as sideplank)
...
function baseExerciseKey(name) {
  const nk = normalizeName(name);
  // Map push-up variants to base 'pushups' key
  if (nk.includes('push')) return 'pushups';
  return nk;
}

export function caloriesForItem(item, userWeightKg = BASE_WEIGHT_KG) {
  // item: { name, reps, sets, durationSec }
  const nameKey = baseExerciseKey(item.name);
